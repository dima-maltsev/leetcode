...
public class RotateArray
{
    ...
    private void NewArray(int[] nums, int k)
    {
    ...
    private void ArrayCopy(int[] nums, int k)
    {
        if (k >= nums.Length) k %= nums.Length;
        if (k == 0 || nums.Length == 1) return;
    ...
    private void SameArrayCopy(int[] nums, int k)
    {
        if (k >= nums.Length) k %= nums.Length;
        if (k == 0 || nums.Length == 1) return;
    ...
    private void ReverseSolution(int[] nums, int k)
    {
        k %= nums.Length;
        if (k == 0 || nums.Length == 1) return;
    ...
    private void Swap2(int[] nums, int k)
    {
        var len = nums.Length;
        k %= len;
        if (k == 0 || len == 1) return;
